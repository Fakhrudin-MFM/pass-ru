...
class BackendApi {
  constructor(options) {
    this.url = 'http://localhost:8888/';// Options.url;
    this.auth = {
      login: 'vasya', // Options.clientLogin,
      password: '123', // Options.clientPassword,
  ...
  profile(token) {
    ...
    return request(reqParams).then(data => JSON.parse(data));
  }
